...
$(function() {
	for (let i = 0; i < prewedimgs.length; ++i) {
		...
		if($("#gallery-prewed") !== null){
			$("#gallery-prewed").append(`<div class="grid-item"><a href="${img.href}?${i}" aria-label="${img.alt}"><img class="lazyload grid-img" alt="${img.alt}" style="height:9em;width:16em;display:unset;" data-src="${img.src}?${i}"></img></a></div>`);
		}
